...
test('extended mixed practice covers five-hour capacity then all 35 ten-minute chapters and seven campaigns',()=>{
 ...
 const p=Core.parentProgress(s);assert.equal(p.totals.practice,25200000);assert.equal(p.activeMs,p.totals.practice+p.totals.math);assert.ok(mono>=445*60000);assert.equal(seen.size,200);assert.ok(enemies.size>=5);assert.equal(s.story.completedChapters.length,7);assert.equal(s.story.clearedAreas.length,35);assert.equal(s.story.chapterComplete,true);assert.ok(s.dragon.xp>=300);assert.equal(s.dragon.stage,3);assert.ok(s.sessions.length>=35);assert.ok(s.sessions.every(x=>x.newWords.length<=9));
