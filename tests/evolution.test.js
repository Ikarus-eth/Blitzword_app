...
test('evolution keeps the XP thresholds, queues each earned stage and changes no learning evidence',()=>{
 const s=C.migrate(C.fresh());s.dragon.xp=13400;const grown=C.migrate(s);
 assert.deepEqual(Content.dragonStages.map(x=>x.xp),[0,3000,8900,13400]);
 ...
 assert.equal(C.beginEvolution(grown),null);assert.equal(grown.dragon.xp,13400);assert.equal(grown.dragon.stage,3);
 ...
 assert.equal(grown.dragon.evolutionSeen,3);assert.equal(grown.dragon.xp,13400);
});
