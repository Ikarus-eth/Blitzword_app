...
test('narration QA has 30 unique representative local clips',()=>{
 assert.equal(qa.SAMPLES.length,30);assert.equal(new Set(qa.SAMPLES.map(x=>x.id)).size,30);
 ...
 assert.equal(groups.get('Gate review'),5);assert.equal(groups.get('Pip evolution'),4);assert.ok(groups.get('Assembled phrase')>=6);
});
