...
Chapter stories are implemented for the 34 entries after the guided first encounter. Each has an illustrated introduction using the existing speech system, followed by one simple untimed sentence for the child to read and confirm. Pause, Home, Rest and reload preserve progress. The chosen dragon name appears in stories, and map/progress labels identify the actual campaign and chapter. See [chapter stories](docs/CHAPTER_STORIES_RELEASE.md). Distinct artwork for every field remains outstanding; existing scenery is reused.
...
Expanded prerecorded narration is the next outstanding audio task; this documentation update does not resume generation or uploads. The app retains 165 recordings and uses browser speech immediately for unrecorded text, without requests for absent files. Recorded word-boundary highlighting also remains outstanding. Enemy artwork and creature-specific health ranges are assigned to the user's parallel thread.
