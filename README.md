...
Recovered prerecorded narration is integrated: all 990 recovered exact-text clips and the four Pip evolution clips (994) are mapped at runtime. A 42-clip representative listening QA found six bad clips; `on`, `creature`, `i`, `it`, standalone `gate`, and the final Pip-evolution line were replaced with user-approved recordings. The other four gate recordings were approved and enabled, and 12 additional older-Tom word samples were all good. Chapter-story and some composite shield-correction phrases still use device speech, and recorded word-boundary highlighting, exhaustive listening and physical iPad playback remain outstanding. See [the narration release](docs/NARRATION_RELEASE.md). Twenty selected enemies now use articulated 2D artwork, and visible health changes wait for the attack's impact. See [enemy assets and animation](docs/enemies/LAYERED_ENEMIES_RELEASE.md). Creature-specific HP ranges, combat groups and distinct growth forms remain proposals.
