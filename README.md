...
Recovered prerecorded narration is integrated: 990 recovered MP3s are preserved and 985 approved exact-text phrases are mapped at runtime. Five gate-family clips remain intentionally fallback-only pending pronunciation approval; chapter-story and some composite shield-correction phrases still use device speech. Personalized dragon names also stay on local device speech. Recorded word-boundary highlighting, listening review and physical iPad playback remain outstanding. See [the narration release](docs/NARRATION_RELEASE.md). Enemy artwork and creature-specific health ranges remain assigned to the user's parallel thread.
