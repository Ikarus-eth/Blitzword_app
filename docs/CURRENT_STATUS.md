...
Documentation reconciled against main `6e7bf84adfa169f0f858c023b5225490acba1967` and the recorded releases. Latest verified runtime build: `enemy-groups-20260925-r2` ([initial enemy release](enemies/GROUP_ENCOUNTERS_DEPLOYMENT.json), [latest live byte verification](heroes/ARCHER_ARMS_DEPLOYMENT.json)); the narration completion draft is not merged or deployed. The earlier cleanup changed documentation only. It closes stale assessment/story/gate approval notices, reconciles current growth/speed rules and removes the obsolete lowercase README that conflicted on case-insensitive filesystems. Historical release evidence is retained.

## Implemented

Archer movement study — 25 September: the user rejected the first prototype's arm movement. The r2 correction separates raising from drawing, moves the drawing elbow outward/back, anchors the hand by the cheek, holds the extended bow arm through release and impact, and lowers the arms independently afterward. The correction is implemented, tested and verified live: [PR #91](https://github.com/Ikarus-eth/Blitzword_app/pull/91), merge `d4470ce961dc620cb4965b7ec8265c578cc67f2c`; [Pages run 36091677491](https://github.com/Ikarus-eth/Blitzword_app/actions/runs/36091677491) succeeded, and eleven published files matched the tested bytes ([verification](heroes/ARCHER_ARMS_DEPLOYMENT.json)). See [the correction record](heroes/ARCHER_ARM_CORRECTION.md) and [review page](https://ikarus-eth.github.io/Blitzword_app/assets/heroes/prototype/archer-review.html). Prototype marker: `archer-study-20260925-r2`; game marker is `enemy-groups-20260925-r2` from the parallel enemy release. The production hero renderer and learner saves remain unchanged. Painted joint seams, final appearance/movement approval, physical iPad checks and production integration remain open. [Initial r1 publication evidence](heroes/ARCHER_DEPLOYMENT.json) is historical, not movement approval.
...
- Number-duel point 10 remains parked: difficulty/adaptation and less guessable choices require discussion. Twenty enemy families are shipped; their approved health ranges, group encounters and age proportions are implemented and locally verified in the group release described above. The fifteen added enemy names still use device speech pending the narration batch.
