...
Build `family-review-complete-20260926-r4` is implemented, tested and deployed in [PR #102](https://github.com/Ikarus-eth/Blitzword_app/pull/102), merge `ada9ff7eacad2341632e9e09df3d0bd828608409`. [Pages run 36207855907](https://github.com/Ikarus-eth/Blitzword_app/actions/runs/36207855907) succeeded, including static-site upload and deployment. All 26 rounds now contain six images (156 total): 31 existing published designs, 119 recovered alternatives and six new concepts completing girl-archer A–E and girl-mage E. Review artwork is included on the website for every device. These are review concepts, not changes to the game's approved production art.

Existing local image assignments and ratings retain their image identity through publication, including a differing local version. Older full backups remain restorable. Import/backup controls are collapsed by default, hero pictures have more room, and there is no missing-image list when every slot is filled. Feedback remains browser-local with explicit export/restore; no cloud rating synchronization is claimed. All 252 automated tests, 89 existing app-flow groups and decoding of all 133 referenced image files pass. Local preview and public browser verification were blocked because the browser tool could not verify its admin-enforced policy. No bypass was attempted. Live rendered layout and post-deployment saved feedback remain unverified; source validation and the successful deployment are recorded separately in [deployment evidence](FAMILY_REVIEW_FINAL_DEPLOYMENT.json). See [release and provenance](FAMILY_REVIEW_FINAL_RELEASE.md).

## Family art review image imports — 25 September 2026

Build `family-review-imports-20260925-r3` is implemented, tested, deployed and verified. The family review now accepts multiple image files, a folder, drag-and-drop or one file directly into an empty slot. Each original is saved separately in browser IndexedDB; identical files are skipped by content hash, so selecting the same files resumes an interrupted import. Exact entity/letter filenames are assigned automatically; generic downloads remain in a saved preview library for manual assignment. A full ZIP backup includes original image bytes, assignments, four reviewers’ ratings and comments and can be restored without a chat session. Browser storage remains device-local; images are not published to GitHub or promoted into production art. The initial published roster remains 31 images with 125 missing alternatives before local imports. Existing feedback and game saves are preserved.

Validation: 13 isolated Chromium scenarios cover migration, imports, reload recovery, exact-byte backup/restore, interrupted batches, multiple tabs, damaged images/archives, storage-full errors and phone/tablet layout. An independent ZIP reader verifies original hashes. All 246 core tests and 89 existing UI-flow groups pass. Physical iPad/Safari remains untested. [PR #100](https://github.com/Ikarus-eth/Blitzword_app/pull/100), merge `52c458e4c923e7e62848877cf0569dae82fb21fa`, was published by [successful Pages run 36144821211](https://github.com/Ikarus-eth/Blitzword_app/actions/runs/36144821211). All four live dashboard files match the tested source byte for byte. See [deployment evidence](FAMILY_REVIEW_IMPORTS_DEPLOYMENT.json) and [use, storage and recovery details](FAMILY_REVIEW_IMPORTS.md).

A subsequent read-only image-cache scan recovered 163 intact unique PNG originals on the user’s Mac. A prepared local folder contains 95 enemy images (five for each previously empty enemy family) and 26 hero candidates; the five already-published Thornling alternatives are excluded. Enemy A–E ordering is newly assigned after visual family identification; original generation labels were not recovered. Hero candidates await manual assignment and may include extra variants. Older concepts, atlases, lineups and screenshots remain separate. The actual 121-file, approximately 260 MB folder imported into an isolated browser on the live dashboard in 3.106 seconds, with 95 automatic placements, 26 unassigned heroes, no errors, and all images retained after reload. This timing is one observed run, not a device-independent guarantee. The recovered originals are in the user’s Downloads folder, not published to GitHub; recovery of every originally generated image is not claimed.
