...
Implemented and locally tested: all 26 fixed sets pass the letter/length and one-letter checks; the similarity heuristic falls from 90.4% to 24.0%. Saved pending options and all calibration rules are preserved. 234 core tests and 83 UI groups pass. Build `assessment-fair-20260924-r1`; deployment verification pending. [Details](ASSESSMENT_CHOICES_RELEASE.md).
...
- Reload and Pip atlas defects: fixed and locally tested; deployment verification pending. Delayed unload storage notifications no longer falsely block a reloaded page; true external changes still block. Explicit atlas clipping prevents neighbouring Pip rows showing in growth/battle. See [release checks](RELOAD_PIP_RELEASE.md).
