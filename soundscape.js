(function(root,factory){
  ...
  function create({AudioContext}={}){
    ...
    function tone(hz,length=.15,volume=.22,delay=0){
      ...
      osc.connect(gain);gain.connect(effects);osc.start(start);osc.stop(start+length+.02);osc.onended=()=>{osc.disconnect();gain.disconnect();};
    }
    return {
      unlock,
      ...
      cue(kind){if(kind==='correct'||kind==='pip'){tone(440,.2);tone(660,.24,.16,.09);}else if(kind==='wrong'){tone(165,.19,.22);tone(123,.24,.16,.08);}else if(kind==='finish'){tone(330,.3);tone(440,.3,.2,.13);tone(660,.4,.18,.26);}},
