...
const clips={};let bytes=0,seconds=0;
for(const clip of corpus.clips){
 ...
 clips[clip.text]={file:clip.file,duration,sha256};bytes+=data.length;seconds+=duration;
}
const manifest={version:'recorded-voice-20260922-r1',voice:generation.voice,language:'en-GB',clips};
...
generation.listeningReview='Not performed in this environment; user listening and physical iPad playback review remain outstanding.';
generation.clipCount=corpus.clips.length;generation.bytes=bytes;generation.durationSeconds=Number(seconds.toFixed(3));
fs.writeFileSync(path.join(root,'docs/NARRATION_GENERATION.json'),JSON.stringify(generation,null,2)+'\n');
console.log(JSON.stringify({clips:corpus.clips.length,bytes,seconds:Math.round(seconds)}));
