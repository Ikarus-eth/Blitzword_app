...
TEXT = "The gate is by the castle."
SETTINGS = {
    "stability": 0.75,
...
def main():
    ...
    audio_path = out / "BlitzWord_gate_sentence_Daniel.mp3"
    if audio_path.exists():
        raise SystemExit("A sample already exists; refusing duplicate generation.")
    quota_request = urllib.request.Request("https://api.elevenlabs.io/v1/user/subscription", headers={"xi-api-key": key})
    with urllib.request.build_opener(NoRedirect).open(quota_request, timeout=30) as response:
        quota = json.load(response)
    remaining = quota["character_limit"] - quota["character_count"]
    if remaining < len(TEXT):
        raise SystemExit("Insufficient existing quota; no speech request sent.")
    print(f"Existing quota available: {remaining}; requested characters: {len(TEXT)}.")
    payload = {
        "text": TEXT,
    ...
    (out / "BlitzWord_gate_sentence_Daniel.json").write_text(json.dumps(metadata, indent=2) + "\n")
