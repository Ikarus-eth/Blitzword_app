...
const result={status:'recording script; see NARRATION_GENERATION.json for generated files and review status',language:'en-GB',direction:'A deep adult male British English voice. Warm, calm, clear natural articulation suitable for children learning to read. Moderate pace, no music, no dramatic character acting, no added words. Keep vowels and final consonants intelligible. Do not artificially pitch-shift recordings.',wordCount:groups.words.length,sentenceCount:groups.sentences.length,clipCount:clips.length,characters:clips.reduce((n,x)=>n+x.text.length,0),clips};
