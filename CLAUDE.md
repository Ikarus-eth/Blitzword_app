...
- Continue the existing app. Preserve learner saves (localStorage key `blitzword_state_v1` and its `_backup`, `_legacy_backup` and `_unreadable_backup` keys) and parallel work. Never reset learner data to roll anything back.
